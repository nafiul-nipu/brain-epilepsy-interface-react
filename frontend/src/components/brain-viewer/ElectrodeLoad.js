...
export const ElectrodeLoad = ({
  ...
  useLayoutEffect(() => {
    ...
    if (visualPanel === "Patch-Com-Net" || visualPanel === "Region-Com-Net") {
      ...
      electrodeData.forEach((electrode, index) => {
        ...
        for (let i = 0; i < currentSample.length; i++) {
          ...
          if (
            currentSample[i].members.includes(electrode["electrode_number"])
          ) {
            meshRef.current.setColorAt(
              index,
              new Color(catColor[currentSample[i].community])
            );
      ...
      electrodeData.forEach((electrode, index) => {
        if (startElec.includes(electrode["electrode_number"])) {
          ...
          const indexOfStartElec = startElec.indexOf(
    ...
    if (eegInBrain !== null) {
      const foundObject = electrodeData.findIndex(
        (obj) => obj.electrode_number === eegInBrain
      );
